...
rPPG 信号の瞬時値 `s(t)` を用いて、顔 ROI 上に半透明の色オーバーレイを Canvas `screen` ブレンドモードで重ねます。

- `s(t) > 0`（収縮期・動脈血充満）→ **動脈赤** `rgba(255, 46, 77, α)` のグラデーション
- `s(t) < 0`（拡張期・脱酸素）→ **酸素シアン** `rgba(61, 250, 255, α)` のグラデーション
- 増幅倍率 `α ∝ |s(t)| × amp` でユーザーが 1〜10 倍に調整可能
- 心拍ごとに画面縁が赤く脈打つ「vignette pulse」アニメーション

これは Wu et al. (2012) の**線形色増幅パイプライン**（ラプラシアンピラミッド + 時間バンドパスフィルタ + 増幅）をブラウザ上で近似実装したものです。
